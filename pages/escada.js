...
function retiraRepeticoesDePersonagem(){
    for(let i = 0; i < db.results[3].fases.length; i++){
        if(db.results[3].fases[i].value == 3)
            db.results[3].fases[i].value = 1;
    }
    localStorage.setItem('db_results_real2', JSON.stringify(db));
}
function funcaoBaixo(){
    retiraRepeticoesDePersonagem();
    db.results[3].fases[Number(db.results[3].index_global)+5].value = 3;
}
function funcaoVerde(){
    db.results[3].fases[db.results[3].index_global].value = 1;
}   
/*funções de movimentação do personagem*/
function funcaoMoveDecrescente(inicio){
    let modo = 0;
    for(var i = 0; i < arranjo_de_ilhas.length; i++){
        if(arranjo_de_ilhas[i] == db.results[3].index_global){
            modo = 1;
        }
    }
    if(modo == 0 && Number(db.results[3].index_global)-1 >= inicio){
        funcaoVerde();
        retiraRepeticoesDePersonagem();
        db.results[3].fases[Number(db.results[3].index_global) - 1].value = 3;
    }
    else{
        funcaoVerde();
        funcaoBaixo();
    ...
    if(db.results[3].index_global == 5 || db.results[3].index_global == 19 || db.results[3].index_global == 9){
        verifica = true;
        if(Number(db.results[3].index_global) == 5)
            window.location.href = "../jogos/jogo2/index2.html";
        if(Number(db.results[3].index_global) == 9)
            window.location.href = "../jogos/jogo1/index1.html";
        if(Number(db.results[3].index_global) == 19)
            window.location.href = "../jogos/jogo3/index3.html";
...
function funcaoMoveCrescente(fim){
    ...
    if(modo == 0 && Number(db.results[3].index_global)+1 <= fim){
        funcaoVerde();
        retiraRepeticoesDePersonagem();
        db.results[3].fases[Number(db.results[3].index_global) + 1].value = 3;
    }
    ...
    if(db.results[3].index_global == 9 || db.results[3].index_global == 19 || db.results[3].index_global == 5){
        verifica = true;
        if(Number(db.results[3].index_global) == 5)
            window.location.href = "../jogos/jogo2/index2.html";
        if(Number(db.results[3].index_global) == 9)
...
function funcaoSairDessaFase(){
    ...
    let comemoracao = ` </br><img id="comemora" src=../images/Escada/comemoracao.gif></br>`;
    $('#res').css("display", "block");
    $('#res').html(comemoracao);
    if( db.results[3].index_global >= 0 && db.results[3].index_global <= 4){
        funcaoMoveCrescente(4);
    }
    else{
        if( db.results[3].index_global >= 15 && db.results[3].index_global <= 19){
            funcaoMoveCrescente(19);
        }
        else{
            if( db.results[3].index_global >= 5 && db.results[3].index_global <= 9){
                funcaoMoveDecrescente(5);
            }
            else{
                if(db.results[3].index_global >= 10 && db.results[3].index_global <= 14){
